	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			files := []struct {
				...
				{"f1", "file1.jsonl", purposeFineTune},
				{"f2", "file2.jsonl", purposeFineTune},
				{"f3", "file3.jsonl", purposeAssistants},
			}
			...
			resp, err := srv.ListFiles(ctx, tc.req)
			if tc.wantErr {
				assert.Error(t, err)
				return
			}
			...
			assert.NoError(t, err)
			assert.Equal(t, tc.wantMore, resp.HasMore)
			assert.Equal(t, int32(3), resp.TotalItems)

			assert.Len(t, resp.Data, len(tc.wantIDs))
			for i, wantID := range tc.wantIDs {
				assert.Equal(t, wantID, resp.Data[i].Id)
			}

			if tc.req.Purpose != "" {
				for _, file := range resp.Data {
					assert.Equal(t, tc.req.Purpose, file.Purpose)
				}
			}
		})
	}
